function r({magnitudeOctaves:t=8,phaseOctaves:e=8,phaseMultiplier:o=1,bias1:i=0,bias2:n=0}={}){var g=`
#ifndef PI
...
vec4 domainColoring (vec4 f_df,
  ...
  float carg = atan(f_df.y, f_df.x) * HALF_PI_INV * ${o.toFixed(1)};
  ...
  ${[...Array(t-1).keys()].map(a=>`float magOctave${a+1} = magOctave${a} * invSteps;`).join(`
  `)}

  ${[...Array(t+1).keys()].map(a=>`float magWeight${a} = ${a===0||a===t?"1e-4":(1+a*i+n*Math.pow(a,2)).toFixed(2)};`).join(`
  `)}
  ...
  ${[...Array(t).keys()].map(a=>`w = mix(magWeight${a}, magWeight${a+1}, 1.0 - logspacing + logtier);
  totalWeight += w;
  gridValue = complexContouringGridFunction(magOctave${a}) * scaleFactor;
  magnitudeGrid += w * smoothstep(width1, width2, gridValue);
  value = fract(-magOctave${a});
  ...
  logspacing = log2(cargGradientMag * ${o.toFixed(1)} * res.y) * invlog2base;
  logspacing = clamp(logspacing, -50.0, 50.0);
  logtier = floor(logspacing);
  ...
  ${[...Array(e-1).keys()].map(a=>`float phaseOctave${a+1} = phaseOctave${a} * invSteps;`).join(`
  `)}

  ${[...Array(e+1).keys()].map(a=>`const float phaseWeight${a} = ${a===0||a===e?"1e-4":(1+a*i+n*Math.pow(a,2)).toFixed(4)};`).join(`
  `)}
  ...
  float phaseGrid = 0.0;
  scaleFactor = pow(steps.y, logtier) / (cargGradientMag * ${o.toFixed(1)}) * 2.0;

  ${[...Array(e).keys()].map(a=>`w = mix(phaseWeight${a}, phaseWeight${a+1}, 1.0 - logspacing + logtier);
  totalWeight += w;
  gridValue = complexContouringGridFunction(phaseOctave${a}) * scaleFactor;
  phaseGrid += w * smoothstep(width1, width2, gridValue);
  value = fract(phaseOctave${a});
  ...
  vec3 result = mix(
    vec3(1),
    rainbow(vec2(carg / ${o.toFixed(1)} - 0.25, shading)).rgb,
    uShadingOpacity
  );
...
}`;return g}export{r as createPolarDomainColoringShader};
