...
fn magma(t: f32) -> vec3<f32> {
  let c0 = vec3<f32>(-0.002, -0.001, -0.006);
  let c1 = vec3<f32>(0.226, 0.041, 0.505);
  let c2 = vec3<f32>(1.089, 0.317, 1.480);
  let c3 = vec3<f32>(-3.06, 1.13, -3.77);
  let c4 = vec3<f32>(4.31, -2.62, 5.30);
  let c5 = vec3<f32>(-2.55, 2.30, -3.06);
  return clamp(c0 + t * (c1 + t * (c2 + t * (c3 + t * (c4 + t * c5)))), vec3<f32>(0.0), vec3<f32>(1.0));
}
