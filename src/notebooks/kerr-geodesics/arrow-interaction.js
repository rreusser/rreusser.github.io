// Arrow interaction handler — drag the arrow tip to change direction,
// or drag the arrow body to translate the initial position.
//
// Strategy: on mousedown, project the arrow tip and shaft to screen space
// and check proximity. Tip takes priority. If hit, suppress camera events
// via capture-phase stopImmediatePropagation, and drag along a screen-space plane.

export function createArrowInteraction(canvas, camera, {
  getArrowState,    // () => { origin: [x,y,z], tip: [x,y,z], visible: boolean }
  onDrag,           // (newTip: [x,y,z]) => void — tip drag
  onBodyDrag,       // (newOrigin: [x,y,z]) => void — body drag (translate position)
  onDragEnd,        // () => void — called when any drag ends
  hitRadius = 40,   // pixel radius for hit-testing
}) {
  let dragging = false;
  let dragMode = null; // 'tip' or 'body'
  ...
  // Distance from point p to line segment A→B in screen space
  ...
  function setupDragPlane(anchorPoint, eye) {
    const viewDir = [
      anchorPoint[0] - eye[0],
      anchorPoint[1] - eye[1],
      anchorPoint[2] - eye[2],
  ...
  function tryStartDrag(sx, sy, e) {
    ...
    // Check tip first (takes priority)
    if (tipScreen) {
      ...
      if (Math.sqrt(dx * dx + dy * dy) <= threshold) {
        ...
        if (ray) {
          ...
          if (worldPt) dragOffset = [state.tip[0] - worldPt[0], state.tip[1] - worldPt[1], state.tip[2] - worldPt[2]];
        }
        return true;
      }
    }

    // Check shaft (line segment from origin to tip)
    if (tipScreen && originScreen) {
      ...
      if (shaftDist <= threshold * 0.6) {
        ...
        if (ray) {
          const worldPt = rayPlaneIntersect(ray.near, ray.far, dragPlaneNormal, dragPlaneD);
          if (worldPt) dragOffset = [state.origin[0] - worldPt[0], state.origin[1] - worldPt[1], state.origin[2] - worldPt[2]];
        }
        return true;
      }
    }
  ...
  function doDrag(sx, sy, e) {
    ...
    if (dragMode === 'tip') {
      onDrag(newPos);
    } else if (dragMode === 'body' && onBodyDrag) {
      onBodyDrag(newPos);
    }
  }
