...
var mouseListen_1;
var hasRequiredMouseListen;

function requireMouseListen () {
	if (hasRequiredMouseListen) return mouseListen_1;
	hasRequiredMouseListen = 1;

	mouseListen_1 = mouseListen;

	var mouse = requireMouse();
	...
	return mouseListen_1;
}

var parseUnit;
var hasRequiredParseUnit;

function requireParseUnit () {
	if (hasRequiredParseUnit) return parseUnit;
	hasRequiredParseUnit = 1;
	parseUnit = function parseUnit(str, out) {
	    if (!out)
	        out = [ 0, '' ];
...
function requireToPx () {
	if (hasRequiredToPx) return toPx;
	hasRequiredToPx = 1;

	var parseUnit = requireParseUnit();
...
var wheel;
var hasRequiredWheel;

function requireWheel () {
	if (hasRequiredWheel) return wheel;
	hasRequiredWheel = 1;

	var toPX = requireToPx();

	wheel = mouseWheelListen;
...
var reglCamera;
var hasRequiredReglCamera;

function requireReglCamera () {
	if (hasRequiredReglCamera) return reglCamera;
	hasRequiredReglCamera = 1;
	var mouseChange = requireMouseListen();
	var mouseWheel = requireWheel();
	var identity = requireIdentity();
	var perspective = requirePerspective();
	var lookAt = requireLookAt();

	reglCamera = createCamera;
...
var reglCameraExports = requireReglCamera();
var createCamera = /*@__PURE__*/getDefaultExportFromCjs(reglCameraExports);
