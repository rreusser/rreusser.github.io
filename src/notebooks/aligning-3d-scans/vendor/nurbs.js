...
var cacheKey = function (nurbs, debug, checkBounds, pointType, weightType, knotType) {
  var d;
  var degreeParts = [];
  var hasAnyKnots = false;
  for (d = 0; d < nurbs.splineDimension; d++) {
    var hasKnots = isArrayLike$3(nurbs.knots) && isArrayLike$3(nurbs.knots[d]);
...
var createVariable$1 = function createVariable (name, nurbs) {
...
var createAccessors$1 = function (nurbs) {
  var accessors = {};
  var accessor;
...
var accessorPreamble$3 = function (nurbs, variableName, propertyName, data) {
  var i;
  var code = [];
...
var evaluate = function (cacheKey, nurbs, accessors, debug, checkBounds, isBasis, derivative) {
  ...
  for (d = 0; d < splineDimension; d++) {
    line('var ' + sizeVar(d) + ' = ' + sizeGetter$2(points, 'this.points', d) + ';');
  }
  code.push(accessorPreamble$2(nurbs, 'x', 'this.points', points));

  if (hasWeights) {
    code.push(accessorPreamble$2(nurbs, 'w', 'this.weights', weights));
  }

  if (!allDimensionUniform) {
    code.push(accessorPreamble$2(nurbs, 'k', 'this.knots', knots));
  }
...
var transform = function createTransform (cacheKey, nurbs, accessors, debug) {
  ...
  code.push('var i, w;');
  code.push(accessorPreamble$1(nurbs, 'x', 'this.points', nurbs.points));

  var sizeVar = variable$1(debug ? 'size' : 's');
  for (i = 0; i < nurbs.splineDimension; i++) {
    code.push('var ' + sizeVar(i) + ' = ' + sizeGetter$1(nurbs.points, 'this.points', i) + ';');
  }
...
var nurbs$1 = /*@__PURE__*/getDefaultExportFromCjs(nurbs_1);

export { nurbs$1 as default };
