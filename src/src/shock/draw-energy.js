...
module.exports = function (regl) {
  return regl({
    vert: `
      ...
      float getEnergy (vec3 U) {
        return (uGamma - 1.0) * (U.z - 0.5 * U.y * U.y / U.x);
      }
  
      void main () {
        vec4 p = vec4(aPosition * 2.0 - 1.0, getEnergy(texture2D(uState, vec2(aPosition, 0.5)).xyz), 0, 1);
        vec4 n = vec4(aNextPosition * 2.0 - 1.0, getEnergy(texture2D(uState, vec2(aNextPosition, 0.5)).xyz), 0, 1);
