...
module.exports = function (regl) {
  return regl({
    vert: `
      ...
      float getDensity (vec3 state) {
        return state.x;
      }
  
      void main () {
        vec4 p = vec4(aPosition * 2.0 - 1.0, getDensity(texture2D(uState, vec2(aPosition, 0.5)).xyz), 0, 1);
        vec4 n = vec4(aNextPosition * 2.0 - 1.0, getDensity(texture2D(uState, vec2(aNextPosition, 0.5)).xyz), 0, 1);
    ...
    frag: `
      precision highp float;
      void main () {
        gl_FragColor = vec4(0.7, 0.1, 0.2, 1);
      }
    `,
    attributes: {
