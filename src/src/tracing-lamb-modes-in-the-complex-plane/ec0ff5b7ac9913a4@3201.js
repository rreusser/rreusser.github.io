...
function _pl(plot,DomainColoringLayer,fLambDispersion,domainColoringConfig,wd,cl,ct,roots,parity,ComplexVariable,html)
{
  ...
  const el = html`<figure class="fullbleed">${pl}<figcaption>Constant-frequency slice of the Lamb wave dispersion relation.</figure>`;
  el.value = pl.value;
  return el;
}
...
function _regl(reglCanvas,viewport,html)
{
  ...
  const el = html`<figure class="fullbleed">${regl}<figcaption>Symmetric (red) and antisymmetric (blue) Lamb wave modes. Click to select/deselect a mode.</figure>`;
  el.value = regl.value;
  return el;
}
...
function _regl2(reglCanvas,viewport,html)
{
  ...
  const el = html`<figure class="fullbleed">${regl}<figcaption>Displacement visualized along the plate cross section.</figure>`;
  el.value = regl.value;
  return el;
}
...
function _plot(PlotContext,viewport){return(
PlotContext({
  yrange: [-20, 20],
  width: viewport.width,
  height: Math.max(400, Math.floor(viewport.width * 0.8))
})
)}
...
function _viewport(figureWidth){return(
{
  width: figureWidth,
  height: Math.floor(Math.max(400, figureWidth * 0.7)),
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("plot")).define("plot", ["PlotContext","viewport"], _plot);
